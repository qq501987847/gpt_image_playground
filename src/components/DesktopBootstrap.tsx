...
import { getDesktopLibraryStatus, initializeDesktopLibrary, reconnectDesktopLibrary, relocateDesktopLibrary, type DesktopLibraryStatus } from '../lib/runtime'
...
export default function DesktopBootstrap({ children }: { children: ReactNode }) {
  ...
  const isRecovering = Boolean(status?.unavailablePath)
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-gray-50 p-6 dark:bg-gray-950">
      <section className="w-full max-w-xl rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-white/[0.08] dark:bg-gray-900">
        <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">{isRecovering ? '素材库暂时不可访问' : '设置素材库'}</h1>
        <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">
          {isRecovering ? `AWAI 未创建新目录，正在等待重新连接已有素材库：${status?.unavailablePath}` : '确认保存位置后，AWAI 才会创建素材目录和数据库。'}
        </p>
        <label className="mt-6 block">
          <span className="mb-1.5 block text-sm text-gray-600 dark:text-gray-300">{isRecovering ? '重新定位已有素材库' : '素材库位置'}</span>
        ...
        <button
          ...
          onClick={() => {
            setLoading(true)
            setError(null)
            const operation = isRecovering ? relocateDesktopLibrary(path.trim()) : initializeDesktopLibrary(path.trim())
            void operation
              .then(setStatus)
              .catch((err) => setError(err instanceof Error ? err.message : String(err)))
              .finally(() => setLoading(false))
        ...
        >
          {loading ? '处理中' : isRecovering ? '连接已有素材库' : '确认并创建'}
        </button>
        {isRecovering && (
          <button
            type="button"
            ...
            onClick={() => {
              setLoading(true)
              setError(null)
              void reconnectDesktopLibrary()
                .then(setStatus)
                .catch((err) => setError(err instanceof Error ? err.message : String(err)))
                .finally(() => setLoading(false))
            }}
            className="mt-3 w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm font-medium text-gray-700 disabled:opacity-50 dark:border-white/[0.12] dark:text-gray-200"
          >
            重试连接
          </button>
        )}
