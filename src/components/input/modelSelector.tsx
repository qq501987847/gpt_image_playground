  ...
  if (compact) {
    ...
    return (
      <div className="relative min-w-0">
        <button type="button" onClick={() => setCompactOpen((open) => !open)} className="flex h-10 min-w-32 max-w-60 items-center gap-1 rounded-xl border border-gray-200/60 bg-white/50 px-2 text-left text-xs text-gray-700 transition-transform active:scale-[0.96] sm:min-w-44 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-200" aria-expanded={compactOpen}>
          <span className="truncate">{keyOptions.find((item) => item.id === activeProfile.keyId)?.name || '选择 Key'}</span>
          <span className="text-gray-300">/</span>
          <span className="truncate">{activeProfile.model || '选择模型'}</span>
        </button>
        {compactOpen && <>
          <button type="button" className="fixed inset-0 z-30 cursor-default" aria-label="关闭 Key 与模型菜单" onClick={() => setCompactOpen(false)} />
          <div className="absolute bottom-full left-0 z-40 mb-2 grid w-[min(34rem,calc(100vw-2rem))] grid-cols-2 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xl dark:border-white/[0.08] dark:bg-gray-900">
