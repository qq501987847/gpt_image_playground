...
export default function InputParamsPanel({
  ...
  if (panel) {
    ...
    return <div className="space-y-3 text-xs">
      {modelSupportsField(capability, 'quality') && <section><p className="mb-1 text-gray-500 dark:text-gray-400">质量</p><div className={`grid gap-2 ${capability.qualities.length > 4 ? 'grid-cols-3' : 'grid-cols-4'}`}>{capability.qualities.map((value) => <button key={value} type="button" onClick={() => setParams({ quality: value })} className={buttonClass(params.quality === value)}>{value === 'auto' ? '自动' : value === 'low' ? '低' : value === 'medium' ? '中' : value === 'high' ? '高' : value === 'xhigh' ? 'xHigh' : 'Max'}</button>)}</div></section>}
